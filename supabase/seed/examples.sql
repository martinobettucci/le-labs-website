/*
  # Example projects seed (DEV ONLY)

  These are the four sample projects that used to be hard-coded in the codebase.
  In dev/demo the stack is pre-seeded with them so the site and back-office have
  realistic content out of the box. Idempotent (ON CONFLICT DO UPDATE).
*/
...
(
  ...
  jsonb_build_object('background', '#23468C', 'color', '#FFFFFF', 'size', 'large'),
  jsonb_build_object(
  ...
  jsonb_build_array(
    jsonb_build_object('hash', 'upd-001-a1b2c3', 'date', '2025-04-10T16:42:00Z', 'title', 'Prototype Testing Results', 'content', 'First round of user testing complete with 87% positive feedback on natural gesture recognition system.'),
    jsonb_build_object('hash', 'upd-001-f6g7h8', 'date', '2025-04-05T09:15:00Z', 'title', 'Development Milestone', 'content', 'Core gesture recognition system now achieves 95% accuracy in controlled environments.')
  )
),
(
  'project-002',
  ...
  jsonb_build_object('background', '#238C33', 'color', '#FFFFFF', 'size', 'medium'),
  jsonb_build_object(
  ...
  jsonb_build_array(
    jsonb_build_object('hash', 'upd-002-k4l5m6', 'date', '2025-04-08T11:30:00Z', 'title', 'New Training Dataset', 'content', 'Expanded training data with 10,000 professional sound design samples for improved stylistic control.')
  )
),
(
  'project-003',
  ...
  jsonb_build_object('background', '#D9CF4A', 'color', '#0D0D0D', 'size', 'medium'),
  jsonb_build_object(
  ...
  jsonb_build_array(
    jsonb_build_object('hash', 'upd-003-p7q8r9', 'date', '2025-03-15T14:20:00Z', 'title', 'Project Completion', 'content', 'Final report published with findings on environmental color adaptation. Source code released as open-source library.')
  )
),
(
  'project-004',
  ...
  jsonb_build_object('background', '#F24141', 'color', '#FFFFFF', 'size', 'small'),
  jsonb_build_object(
  ...
  jsonb_build_array(
    jsonb_build_object('hash', 'upd-004-u1v2w3', 'date', '2025-04-01T09:45:00Z', 'title', 'Research Plan Approved', 'content', 'Project plan and initial research methodology approved. Initial prototype development to begin next week.')
  )
)
